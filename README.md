...
4. **BeanPostProcessor (Before Initialization, optional)** : Any registered `BeanPostProcessor`s are applied before the bean’s initialization callbacks.
...
6. **BeanPostProcessor (After Initialization, optional)** : Any registered `BeanPostProcessor`s are applied after the bean’s initialization callbacks.
...
These mechanisms are explained in the next sections: [Bean Creation and Initialization](#bean-creation-and-initialization) and [Bean Destruction and Cleanup](#bean-destruction-and-cleanup).
