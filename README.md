...
* [Practical Example: Simple Layered Architecture](#practical-example-simple-layered-architecture)
...
This allows developers to focus on business logic rather than on object creation and dependency management.
...
Although XML configuration remains supported for legacy applications, modern Spring Boot projects typically favor annotation-driven and Java-based configurations
for better readability, maintainability, and seamless integration with auto-configuration.
...
For now, XML is still supported, but I think it is mainly used in legacy systems or specific integration scenarios.

## Annotation-Based Configuration

Annotation-based configuration allows developers to mark classes with specific annotations, enabling Spring to automatically detect and register them as beans during component scanning.
I think this is the most common and convenient way to define beans in modern Spring applications

Developers typically use [Spring Stereotypes](#spring-stereotypes), such as:
...
Java-based configuration defines beans using configuration classes annotated with `@Configuration` and methods annotated with `@Bean`.
...
However, it can become verbose and harder to maintain as the application grows, which is why I think modern Spring Boot projects prefer annotation-driven or Java-based configuration.
...
* **[@Component](#component)**: General-purpose annotation for any Spring-managed bean.
* **[@Repository](#repository)**: Marks a class as a data access or persistence layer component. 
* **[@Service](#service)**: Indicates that the class holds business logic or service operations.
* **[@Controller](#controller)**: Marks a class as a web controller, typically used in Spring MVC to handle HTTP requests.
* **[@RestController](#restcontroller)**: A combination of `@Controller` and `@ResponseBody`, used to create RESTful web services.
...
Marks a persistence (data access) component, including Spring Data repository interfaces.
Spring can automatically translate persistence-related exceptions into its unified `DataAccessException` hierarchy.
...
This rendering is handled by a template engine such as Thymeleaf.
...
The same template is used for both all orders and valid orders, with the displayed data changing based on the controller method that handles the request.
...
The template will display either all orders or only valid orders, depending on which URL is accessed.
...
* **[Singleton Scope](#singleton-scope)**: The default Spring scope, with one shared instance per container. Useful for stateless services and shared resources.
* **[Prototype Scope](#prototype-scope)**: A new instance is created every time the bean is requested. Useful for stateful or temporary objects.
* **[Request Scope](#request-scope)**: A new instance is created for each HTTP request. Useful for request-specific data.
* **[Session Scope](#session-scope)**: One instance per HTTP session, shared across multiple requests from the same user. Useful for user-specific state.
* **[Application Scope](#application-scope)**: One instance per web application context, shared across all requests and sessions. Useful for global resources or caches.
* **[WebSocket Scope](#websocket-scope)**: One instance per WebSocket session. Useful for maintaining session-specific state in real-time applications.

For my thoughts on when to use each scope, please see [When to Use Each Scope](#when-to-use-each-scope).

### Singleton Scope

Singleton is the default scope in Spring. Only one shared instance of the bean is created per Spring IoC container.
Every time the bean is requested from the same container, the same instance is returned.
...
Within the same WebSocket session, all injections of `WebSocketService` share the same instance.
...
# Spring Bean Lifecycle (Singleton Beans)
...
3. **Aware Interfaces**  
   If the bean implements certain aware interfaces, the container supplies the corresponding context information.
   These include `BeanNameAware`, `BeanFactoryAware`, and `ApplicationContextAware`.
...
5. **Initialization**  
   The Spring container calls the bean's configured initialization logic,
   e.g. `@PostConstruct`, `afterPropertiesSet()` from `InitializingBean`, or a custom `init-method`.
6. **BeanPostProcessor (After Initialization)**  
   Registered `BeanPostProcessors` are applied after the bean's initialization callbacks. 
   For example, `postProcessAfterInitialization()`.
7. **Ready for Use**  
   The bean is fully initialized and available for injection or retrieval from the container.
8. **Destruction**  
   When the application context is closed, Spring invokes destruction callbacks. 
   This can include `@PreDestroy`, `destroy()` from `DisposableBean`, or a custom `destroy-method`. 
   Prototype beans are not automatically destroyed by the container.

**Note:** The step names above are descriptive labels used to explain the observable lifecycle behavior of a bean managed by the Spring container.
I did not find any official Spring documentation that defines a formal, named set of lifecycle phases.
...
For details on bean lifecycle, initialization, and destruction, including non-singleton scopes, see the [Spring Framework Documentation](https://docs.spring.io/spring-framework/).

### Conceptual Flow
Before diving into the implementation details, it might be helpful to see the entire lifecycle at a glance.
...
```
Instantiation → Dependency Injection → Aware Interfaces → BeanPostProcessor (Before Initialization) →
Initialization → BeanPostProcessor (After Initialization) → Ready for Use → Destruction
```
...
* interfaces such as `InitializingBean`, `DisposableBean`, 
...
* [Initialization with Annotations](#initialization-with-annotations)
* [Initialization with Interfaces](#initialization-with-interfaces)
* [Initialization with Init Method](#initialization-with-init-method)

For my thoughts on choosing an initialization approach, please see [Choosing the Right Way to Initialize a Bean](#choosing-the-right-way-to-initialize-a-bean).
...
* **Annotating a method with `@PostConstruct`**:  I like this approach because it keeps the bean decoupled from Spring and the code cleaner to maintain.
...
This cleanup allows beans to release resources, close connections, or perform any necessary shutdown operations before being removed from the container.

Spring provides various ways to define destruction logic, depending on how beans are configured:
* [Destruction with Annotations](#destruction-with-annotations)
* [Destruction with Interfaces](#destruction-with-interfaces)
* [Destruction with Destroy Method](#destruction-with-destroy-method)

For more details, see:
* [When Bean Destruction Happens](#when-bean-destruction-happens)
* [Choosing the Right Way to Clean Up a Bean](#choosing-the-right-way-to-clean-up-a-bean)

### Destruction with Annotations

The common and modern approach is to use the `@PreDestroy` annotation.
This annotation marks a method that the Spring container will call before the bean is destroyed, e.g. when the application context is being closed for singleton beans.
...
In Java configuration classes, it is possible to specify a `destroyMethod`.
...
```xml
<bean id="someCustomDestroyBean" class="pl.kamilmazurek.example.beans.mybean.MyCustomDestroyBean" destroy-method="customDestroy"/>
```
...
For more details please see: [Customizing the Nature of a Bean](https://docs.spring.io/spring-framework/reference/7.0/core/beans/factory-nature.html).
...
No matter which approach you choose, proper cleanup is essential to ensure that resources such as file handles, database connections, or threads are released safely.
...
Spring Data JPA manages repository beans, creating proxy implementations at runtime.
...
If you are looking for practical architecture templates, please see:
